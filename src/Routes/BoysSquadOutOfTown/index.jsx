...
const BoysSquadOutOfTown = ({settings}) => {
    const [models, setModels] = useState([]);
    const [pageData, setPageData] = useState([]);
    const [showListItem, setShowListItem] = useState(false);
    const { rowCount } = settings.boysSquaPagesSettings || 3;
    ...
    const fetchPageData = async () => {
        try {
            const query = `*[_type == 'boysSquadOutOfTownPage'][0]`;
    ...
    const fetchModelData = async () => {
        try {
            const query = `*[_type == 'boysSquadModels' && defined(modelStats) && defined(modelPictures) && status == 'out-of-town'] {
                modelName,
                modelStats,
                modelPictures
            }`;
    ...
    return (
        <>
            <main className='boysSquad-outoftown-page'>
...
export default BoysSquadOutOfTown;